...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // phát âm thanh riêng cho mỗi chế độ
  if (willBeDark) {
    sounds.darkMode.play();  // bật dark
  } else {
    sounds.lightMode.play(); // bật light
  }
  ...
  // cập nhật icon + các thành phần liên quan
  themeIcon.src = dark
    ? "./img/dark_mode_dark.webp"
    : "./img/dark_mode_light.webp";
  applyThemeIcons();
...
// === Easter player (giữ như bạn đã có) ===
const EasterPlayer = window.EasterPlayer ?? {
  sound: new Howl({
    src: ['./sounds/rework.mp3'],
    volume: 0.7,
    loop: true,
    preload: true
  }),
  ...
  toggle() {
    if (this.playing) {
      ...
      if (Howler._muted) return; // tôn trọng nút mute
      this.sound.play();
      this.playing = true;
      updateStarUI(true);
    }
...
function updateStarUI(playing){
  // hiệu ứng nhỏ và đổi label
  if (starBtn) {
    starBtn.style.transform = playing ? 'scale(1.05) rotate(10deg)' : '';
    setTimeout(() => starBtn.style.transform = '', 300);
  }
  if (starTag) {
    starTag.textContent = playing ? 'dừng nhạc' : 'fun tuff';
  }
}
...
starTag?.addEventListener('click', (e) => {
  e.preventDefault();
  EasterPlayer.toggle();
});
