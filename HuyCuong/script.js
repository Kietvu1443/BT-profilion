...
;(() => {
  const pill = document.querySelector('.strengths .pill.featured');
  if (!pill) return;

  // Wrap existing text into a .content span so sparks/ripples layer correctly
  if (!pill.querySelector('.content')) {
    const content = document.createElement('span');
    content.className = 'content';
    while (pill.firstChild) content.appendChild(pill.firstChild);
    pill.appendChild(content);
  }

  // Add three orbiting sparks
  for (let i=0;i<3;i++){ pill.appendChild(document.createElement('span')).className='spark'; }
  ...
  pill.addEventListener('pointermove', (e)=>{
    ...
    pill.style.setProperty('--mx', mx.toFixed(1) + 'px');
    pill.style.setProperty('--my', my.toFixed(1) + 'px');
    pill.classList.add('magnet');
  });
  pill.addEventListener('pointerleave', ()=>{
    pill.classList.remove('magnet');
...
// Smooth-scroll "Xem dự án" → #projects (with sticky topbar offset)
(() => {
  const target = document.querySelector('#projects');
  const topbar = document.querySelector('.topbar');
  const links = Array.from(document.querySelectorAll('a[href="#projects"], .btn-xem-du-an'));
  function go(){
    if (!target) return;
  ...
  links.forEach(a => a.addEventListener('click', (e) => { e.preventDefault(); go(); }));
})();



;(()=>{
  const cards = Array.from(document.querySelectorAll('#gallery .card.hover-zoom'));
  ...
  cards.forEach(card => {
    function enter(){ card.classList.add('active'); }
    function move(e){
      ...
      card.style.setProperty('--tx', (x*30).toFixed(1)+'px');
      card.style.setProperty('--ty', (y*30).toFixed(1)+'px');
    }
    function leave(){
      card.classList.remove('active');
      card.style.setProperty('--rx','0deg'); card.style.setProperty('--ry','0deg');
      card.style.setProperty('--tx','0px');  card.style.setProperty('--ty','0px');
    }
  ...
  function openLightbox(src){
    if (!lb){
      ...
      lb.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,.85);display:flex;align-items:center;justify-content:center;z-index:10000;opacity:0;transition:opacity .25s;';
      const img = document.createElement('img');
      img.style.cssText = 'max-width:92vw;max-height:88vh;border-radius:14px;box-shadow:0 30px 80px rgba(0,0,0,.55)';
      lb.appendChild(img);
  ...
  cards.forEach(card => {
    ...
    card.addEventListener('click', ()=> openLightbox(img.getAttribute('src')) );
  });
})();
